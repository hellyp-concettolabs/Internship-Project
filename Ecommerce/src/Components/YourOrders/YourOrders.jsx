...
function YourOrders() {
    ...
    const fetchYourOrder = async () => {
      ...
      await axios.post(`${Domain_Base_Url}/my-order-list`,{
    ...
    const handleCancelOrder = async(order_id) =>{
      axios.defaults.headers.common['Authorization'] = `Bearer ${localStorage.getItem("token")}`;
      await axios.post(`${Domain_Base_Url}/order-cancel`,{
            ...
            <div key={i} className=" py-1 border-bottom d-flex flex-column ">
                ...
                <div className="orderitemcontainer">
                  <div className="orderimgtitle ">
                    ...
                    <div className="orderiteminfo d-flex flex-column gap-2">
                        <div className="orderitemtitle"
                          style={{cursor:"pointer"}}
                          onClick={() => navigate(`/orders/${d.sub_order_number}/${d.id}`)}>
                                <SingleProductTitle name={d.product_info.name}/>
                        </div>
                        <div className=" fw-bold orderitemprice"><sup>$</sup>{d.product_variation_id === null ? d.product_info.sale_price : d.product_info.product_variation_detail[0].sale_price}</div>
                        <div className=" d-flex gap-3 ">
                            ...
                            <div key={i}>
                                <span className="orderitemtype fw-semibold text-secondary ">{pv.variation_type}: </span> 
                                <b>{pv.variation_name}</b>
                            </div>
                            )))}
                            <div>
                                <span className="orderitemprice fw-semibold text-secondary ">Quantity:</span> <b>{d.quantity}</b>
                            </div>
                        </div>
                    </div>
                  </div>
